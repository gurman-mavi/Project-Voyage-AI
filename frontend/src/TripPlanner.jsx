...
/**
 * TripPlanner.jsx
 * - Form: origin, destination, dates, budget, adults, interests
 * - POSTs to /api/agent/plan
 * - Renders plan options (flight, hotel, daily plan with weather)
 * - Destination banner via /api/images/destination (with credit overlay)
 * - Persists last form in localStorage
 */

/* ============================ Constants & Utils ============================ */

const INTERESTS = ["Culture", "Food", "Shopping", "Nightlife", "Beach"];
...
/** Autocomplete for cities/IATA from /api/destinations/suggest */
function CityIataAutocomplete({
  ...
  // keep internal input synced with parent prop
  ...
  // fetch suggestions after minChars
  useEffect(() => {
    ...
    <div className="relative" ref={boxRef}>
      ...
      <input
        ...
        onChange={(e) => {
          ...
          setInput(v);
          setOpen(true);
          onTyping?.(v); // keep parent form state live while typing
        }}
      ...
      <header className="border-b border-slate-100 bg-gradient-to-b from-white to-slate-50">
        <div className="max-w-6xl mx-auto px-4 py-6">
          ...
          <p className="text-slate-600">
            Give us the basics — we’ll draft bookable options and a day-by-day plan.
          </p>
        </div>
      </header>
      ...
      <section className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="mt-3">
          ...
          <div className="mt-3 flex items-center gap-3">
            <button
              onClick={submit}
              className="px-4 py-2 rounded-xl font-semibold border border-indigo-300 bg-gradient-to-b from-white to-indigo-50 hover:border-indigo-400 text-indigo-800"
              disabled={loading}
            >
...
const imgCache = new Map(); // key: city|country -> {url, credit, sourceUrl}

function Banner({ destinationToken }) {
  const [cityMeta, setCityMeta] = useState(null); // { cityName, country }
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        // Try to resolve from catalog (if token is IATA)
        const r = await fetch(
        ...
        // Ask backend image service (wikipedia/unsplash)
        const ri = await fetch(
          `/api/images/destination?city=${encodeURIComponent(cityName)}&country=${encodeURIComponent(
            country
          )}`
        );
